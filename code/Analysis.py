...
class Analysis:
    ...
    def significance(self, weights, lum=140e3, res=0.0001, plot=True, path=None):
        ...
        for idx, threshold in enumerate(thresholds):
            ...
            sg[idx] = (lum * weights * 5 * ((self.y_pred >= threshold) & (self.y_test.values[:,-1] == 1)).astype(int)).sum()
            bg[idx] = (lum * weights * 5 * ((self.y_pred >= threshold) & (self.y_test.values[:,-1] == 0)).astype(int)).sum()
        # Compute Asimov estimates and propagate statistical uncertainty
        Z_0, Z_5, Z_10, Z_20 = Z(sg, bg, sig=0.), Z(sg, bg, sig=0.05), Z(sg, bg, sig=0.1), Z(sg, bg, sig=0.2) 
        Z_err_0, Z_err_5, Z_err_10, Z_err_20 = eZ(sg, bg, sig=0.), eZ(sg, bg, sig=0.05), eZ(sg, bg, sig=0.1), eZ(sg, bg, sig=0.2)
        if plot:
            fig, ax = plt.subplots()
            ax.plot(thresholds, Z_0, color='b', label=r"$\sigma_b=0\%$")
            ax.fill_between(thresholds, Z_0-Z_err_0, Z_0+Z_err_0, alpha=0.25, color='b')
            ax.plot(thresholds, Z_5, color="r", label=r"$\sigma_b=5\%$")
            ax.fill_between(thresholds, Z_5-Z_err_5, Z_5+Z_err_5, alpha=0.25, color='r')
            ax.plot(thresholds, Z_10, color="g", label=r"$\sigma_b=10\%$")
            ax.fill_between(thresholds, Z_10-Z_err_10, Z_10+Z_err_10, alpha=0.25, color='g')
            ax.set_xlabel("Cut on the Discriminator", fontsize=12)
            ax.set_ylabel(r"$Z_A$", fontsize=12)
            ax.set_xticks(np.arange(0, 1.05, 0.05))
            ax.set_xticklabels([f"{j:.1f}" if i % 4 == 0 else "" for i,j in enumerate(np.arange(0, 1.05, 0.05))])
